...
# 목표수질 초과 여부
총량측정망_원본['달성여부'] = np.where(총량측정망_원본['TP'] > 목표수질_TP, "초과", "달성")
...
## 수질측정망 자료 불러오기
수질측정망 = (
    pd.read_excel("E:/Coding/TMDL/수질분석/수질측정망/주천A_수질측정망_2015_2024.xlsx")
    .dropna(subset=['TP'])  # TP 결측 제거
)

# TP 2 이상인 행 제거(이상수치 제외)
#수질측정망 = 수질측정망.query("TP < 2")
...
수질측정망_평균 = (
    ...
    .agg(TP_평균=('TP', lambda x: np.round(x.mean(skipna=True), 3)))  # ④ 평균 + 반올림
)
...
# 저장
plt.savefig("E:/Coding/TMDL/수질분석/Output/Plot/주천A_상관계수.png", dpi=300)
plt.show()
...
for x, label in 유황라벨.items():
    ax1.text(x, 1.3, label, ha='center', va='center', fontsize=12, weight='bold')
...
# 두 그룹간 평균 비교
mean_rain = tp_rain.mean()
mean_dry = tp_dry.mean()
...
# 📦 박스플롯 (이상치 표시 없이)
sns.boxplot(
    data=데이터, x='측정소명', y='TP', order=order,
    ax=ax, showfliers=False, width=0.5,
    color='white', linewidth=1,
    linecolor='black'
)
...
# 🔴 목표수질 수평선
ax.axhline(y=목표수질_TP, color='red', linestyle='--', 
