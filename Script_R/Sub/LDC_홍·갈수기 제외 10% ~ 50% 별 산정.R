...
FLOW <- FLOW %>% select(총량지점명, 갈수기, 풍수기)
OBS <- OBS %>% left_join(FLOW, by = "총량지점명")
OBS1 <- OBS %>% filter(유량 > 갈수기 & 유량 <= 풍수기)

# 연도별 초과율에 따른 수질 구하기 
for(i in 2005:2016) {
# BOD -----------------------------------------------------------------------------------------------------------------------
    assign(paste0("BOD_", i, "_", i+2), OBS1 %>%
    group_by(총량지점명) %>%                                              # 지점별 분리
    filter(년 %in% c(i, i+1, i+2)) %>%                                    # 해당연도만 추출
    select(총량지점명, 년, BOD, TP) %>% 
    mutate(BOD_RANK = rank(-BOD, ties.method = c("first"))) %>%           # 데이터 순위 매기기 
    mutate(Total = n ()) %>%                                              # 전체 데이터 갯수 세기
    mutate(a.10 = floor(Total * 0.1 + 1),                                 # 각 초과율에 해당하는 데이터 순위 확인
           a.20 = floor(Total * 0.2 + 1),
           a.30 = floor(Total * 0.3 + 1),
           a.40 = floor(Total * 0.4 + 1),
           a.50 = floor(Total * 0.5 + 1)) %>% 
...
# TP ----------------------------------------------------------------------------------------------------------------------- 
  assign(paste0("TP_", i, "_", i+2), OBS1 %>%
           group_by(총량지점명) %>%                                        # 지점별 분리
           filter(년 %in% c(i, i+1, i+2)) %>%                              # 해당연도만 추출
           select(총량지점명, 년, BOD, TP) %>% 
...
# 연도별 데이터 하나로 합치기
BOD_group <- bind_rows(BOD_2005_2007, BOD_2006_2008, BOD_2007_2009, BOD_2008_2010, 
                       BOD_2009_2011, BOD_2010_2012, BOD_2011_2013, BOD_2012_2014,
                       BOD_2013_2015, BOD_2014_2016, BOD_2015_2017, BOD_2016_2018)

TP_group <- bind_rows(TP_2005_2007, TP_2006_2008, TP_2007_2009, TP_2008_2010,             
                      TP_2009_2011, TP_2010_2012, TP_2011_2013, TP_2012_2014,
                      TP_2013_2015, TP_2014_2016, TP_2015_2017, TP_2016_2018)
...
# 데이터 csv로 내보내기
write.csv(TP_group, file = "D:/1. 수질총량/3. 목표수질/총량측정망 수질 현황 정리/TP_LDC.csv")
write.csv(BOD_group, file = "D:/1. 수질총량/3. 목표수질/총량측정망 수질 현황 정리/BOD_LDC.csv")
