...
## 분류 및 수거만 남기고 나머지는 0으로 수정(합류, 오수, 단독은 P2-1과 P2-2에 정리)
P1.population <- population2 %>%
  mutate(
    시가합류식인구 = 0, 시가오수처리인구 = 0, 시가단독정화인구 = 0,
    비시가합류식인구 = 0, 비시가오수처리인구 = 0, 비시가단독정화인구 = 0
  ) %>%
  # 합류, 오수, 단독 제외로 합계 재산정
  mutate(총인구 = pmap_dbl(select(., 시가분류식인구:시가수거식인구, 비시가분류식인구:비시가수거식인구), sum))
...
#
# ## ID 정리(행정구역코드별로 ID 부여하되 한 동리에서 둘 이상의
# ## 환경기초시설로 유입되는 경우 각각 ID 부여)
# population_id <- population1 %>%
#   # 인구 자료에서 행정구역 코드와 처리시설명만 선택
#   select(행정구역코드, 시가하수처리시설명) %>%
#   arrange(행정구역코드) %>%
#   # 처리시설이 위치한 행정구역 코드 추가
#   left_join(stp_code, by = "시가하수처리시설명") %>%
#   mutate_at(vars(처리시설행정구역코드), ~ replace(., is.na(.), 0)) %>%
#   # 각 동리의 행정구역코드와 해당 동/리와 연계된 처리시설행정구역코드 합쳐서 처리시설위치확인 코드 생성
#   # 동리에 두개 이상 처리시설이 있을 경우 처리시설위치확인 코드가 다른 경우
#   # 각기 다른 동리에 위치한 처리시설로 유입 된다고 보고 각각 다른 ID 부여
#   mutate(처리시설위치확인 = str_c(행정구역코드, 처리시설행정구역코드)) %>%
#   # 처리시설위치확인 코드가 동일한 경우 단일 ID 부여 되도록 중복 자료 삭제
...
## 분류 및 수거만 남기고 나머지는 0으로 수정(합류, 오수, 단독은 P2-1과 P2-2에 정리)
P2_0.waterusage <- waterusage2 %>%
  mutate(
    가정시가합류 = 0, 가정시가오수 = 0, 가정시가단독 = 0,
    가정비시가합류 = 0, 가정비시가오수 = 0, 가정비시가단독 = 0,
    영업시가합류 = 0, 영업시가오수 = 0, 영업시가단독 = 0,
    영업비시가합류 = 0, 영업비시가오수 = 0, 영업비시가단독 = 0
  ) %>%
  # 합류, 오수, 단독 제외로 합계 재산정
  mutate(
    가정용물사용량계 = pmap_dbl(select(., 가정시가분류:가정비시가수거), sum),
    영업용물사용량계 = pmap_dbl(select(., 영업시가분류:영업비시가수거), sum),
    물사용량합계 = 가정용물사용량계 + 영업용물사용량계
  ) %>%
  # 필요한 항목만 선택
  select(-하수처리시설명, -분뇨처리시설명, -지정내역)

## 파일 내보내기
write_xlsx(P2_0.waterusage, path = "Output/Data/부하량산정 기초자료/P2생활계물사용량.xlsx")


### 기본계획 오수처리시설 및 정화조 비율 반영 ----------------------------------
## 오수처리 자료 정리
population_2017 <- read_excel("Data/부하량 검토/생활계검토/생활계가정인구현황.xlsx") %>%
  filter(시도 == "강원도") %>%
  select(
    행정구역코드, 시가분류식인구, 시가수거식인구,
    비시가분류식인구, 비시가수거식인구
  ) %>%
  group_by(행정구역코드) %>%
  summarise_at(
  ...
  mutate(
    인구비율_시가분류식 = 시가분류식인구 / (시가분류식인구 + 시가수거식인구),
    인구비율_시가수거식 = 시가수거식인구 / (시가분류식인구 + 시가수거식인구),
    인구비율_비시가분류식 = 비시가분류식인구 / (비시가분류식인구 + 비시가수거식인구),
    인구비율_비시가수거식 = 비시가수거식인구 / (비시가분류식인구 + 비시가수거식인구)
  ) %>%
  select(-c(시가분류식인구, 시가수거식인구, 비시가분류식인구, 비시가수거식인구)) %>%
  mutate_at(vars(행정구역코드), as.character)


waterusage_2017 <- read_excel("Data/부하량 검토/생활계검토/생활계물사용량.xlsx") %>%
  filter(시도 == "강원도") %>%
  select(
    행정구역코드, 가정시가분류, 가정시가수거, 가정비시가분류, 가정비시가수거,
    영업시가분류, 영업시가수거, 영업비시가분류, 영업비시가수거
  ) %>%
  group_by(행정구역코드) %>%
  summarise_at(
    vars(
      가정시가분류, 가정시가수거, 가정비시가분류, 가정비시가수거,
      영업시가분류, 영업시가수거, 영업비시가분류, 영업비시가수거
    ),
    ~ sum(.)
  ) %>%
  mutate(
    유량비율_가정시가분류 = 가정시가분류 / (가정시가분류 + 가정시가수거),
    유량비율_가정시가수거 = 가정시가수거 / (가정시가분류 + 가정시가수거),
    유량비율_가정비시가분류 = 가정비시가분류 / (가정비시가분류 + 가정비시가수거),
    유량비율_가정비시가수거 = 가정비시가수거 / (가정비시가분류 + 가정비시가수거),
    유량비율_영업시가분류 = 영업시가분류 / (영업시가분류 + 영업시가수거),
    유량비율_영업시가수거 = 영업시가수거 / (영업시가분류 + 영업시가수거),
    유량비율_영업비시가분류 = 영업비시가분류 / (영업비시가분류 + 영업비시가수거),
    유량비율_영업비시가수거 = 영업비시가수거 / (영업비시가분류 + 영업비시가수거)
  ) %>%
  select(-c(
    가정시가분류, 가정시가수거, 가정비시가분류, 가정비시가수거,
    영업시가분류, 영업시가수거, 영업비시가분류, 영업비시가수거
...
individual_STP_2017 <- read_excel("Data/부하량 검토/생활계검토/오수처리시설.xlsx") %>%
  filter(시도 == "강원도") %>%
  select(행정구역코드, 지역구분, 업종, 인구, 사용유량) %>%
  mutate(
    업종 = case_when(
      업종 == "가정용" ~ "가정",
      업종 == "영업용" ~ "영업"
  ...
  group_by(행정구역코드, 구분) %>%
  summarise(인구 = sum(인구), 유량 = round(sum(사용유량), 2), .groups = "drop") %>%
  pivot_wider(
    names_from = 구분,
    values_from = c(인구, 유량)
  ) %>%
  mutate_at(vars(2:9), ~ replace_na(., 0))


septictank_2017 <- read_excel("Data/부하량 검토/생활계검토/정화조.xlsx") %>%
  filter(시도 == "강원도") %>%
  select(행정구역코드, 지역구분, 업종, 인구, 사용유량) %>%
  mutate(
    업종 = case_when(
...
iSTP_septic_2017 <- individual_STP_2017 %>%
  left_join(septictank_2017, by = "행정구역코드") %>%
  mutate_at(vars(10:17), ~ replace_na(., 0)) %>%
  mutate(
    인구비율_시가오수 = 인구_가정시가오수 / (인구_가정시가오수 + 인구_가정시가단독),
    인구비율_비시가오수 = 인구_가정비시가오수 / (인구_가정비시가오수 + 인구_가정비시가단독),
    유량비율_가정시가오수 = 유량_가정시가오수 / (유량_가정시가오수 + 유량_가정시가단독),
    유량비율_가정비시가오수 = 유량_가정비시가오수 / (유량_가정비시가오수 + 유량_가정비시가단독),
    유량비율_영업시가오수 = 유량_영업시가오수 / (유량_영업시가오수 + 유량_영업시가단독),
    유량비율_영업비시가오수 = 유량_영업비시가오수 / (유량_영업비시가오수 + 유량_영업비시가단독),
    인구비율_시가단독 = 인구_가정시가단독 / (인구_가정시가오수 + 인구_가정시가단독),
    인구비율_비시가단독 = 인구_가정비시가단독 / (인구_가정비시가오수 + 인구_가정비시가단독),
    유량비율_가정시가단독 = 유량_가정시가단독 / (유량_가정시가오수 + 유량_가정시가단독),
    유량비율_가정비시가단독 = 유량_가정비시가단독 / (유량_가정비시가오수 + 유량_가정비시가단독),
    유량비율_영업시가단독 = 유량_영업시가단독 / (유량_영업시가오수 + 유량_영업시가단독),
    유량비율_영업비시가단독 = 유량_영업비시가단독 / (유량_영업비시가오수 + 유량_영업비시가단독)
  ) %>%
  select(-c(2:17))


population2 <- population1 %>%
  left_join(population_2017, by = "행정구역코드") %>%
  left_join(iSTP_septic_2017, by = "행정구역코드") %>%
  mutate_at(vars(인구비율_시가분류식, 인구비율_비시가분류식), ~ replace_na(., 1)) %>%
  mutate_at(vars(인구비율_시가수거식, 인구비율_비시가수거식), ~ replace_na(., 0)) %>%
  mutate_at(
    vars(
      인구비율_시가오수, 인구비율_비시가오수, 유량비율_가정시가오수,
      유량비율_가정비시가오수, 유량비율_영업시가오수, 유량비율_영업비시가오수
    ),
    ~ replace_na(., 1)
  ) %>%
  mutate_at(
    vars(
      인구비율_시가단독, 인구비율_비시가단독, 유량비율_가정시가단독,
      유량비율_가정비시가단독, 유량비율_영업시가단독, 유량비율_영업비시가단독
    ),
    ~ replace_na(., 0)
  ) %>%
  mutate(
    시가분류식인구 = round((시가분류식인구 + 시가수거식인구) * 인구비율_시가분류식, 0),
    시가수거식인구 = round((시가분류식인구 + 시가수거식인구) * 인구비율_시가수거식, 0),
    비시가분류식인구 = round((비시가분류식인구 + 비시가수거식인구) * 인구비율_비시가분류식, 0),
    비시가수거식인구 = round((비시가분류식인구 + 비시가수거식인구) * 인구비율_비시가수거식, 0),
    시가오수처리인구 = round((시가오수처리인구 + 시가단독정화인구) * 인구비율_시가오수, 0),
    시가단독정화인구 = round((시가오수처리인구 + 시가단독정화인구) * 인구비율_시가단독, 0),
    비시가오수처리인구 = round((비시가오수처리인구 + 비시가단독정화인구) * 인구비율_비시가오수, 0),
    비시가단독정화인구 = round((비시가오수처리인구 + 비시가단독정화인구) * 인구비율_비시가단독, 0)
  ) %>%
  select(-c(28:43))


waterusage2 <- waterusage1 %>%
  left_join(waterusage_2017, by = "행정구역코드") %>%
  left_join(iSTP_septic_2017, by = "행정구역코드") %>%
  mutate_at(
    vars(
      유량비율_가정시가분류, 유량비율_가정비시가분류,
      유량비율_영업시가분류, 유량비율_영업비시가분류
    ),
    ~ replace_na(., 1)
  ) %>%
  mutate_at(
    vars(
      유량비율_가정시가수거, 유량비율_가정비시가수거,
      유량비율_영업시가수거, 유량비율_영업비시가수거
    ),
    ~ replace_na(., 0)
  ) %>%
  mutate_at(
  ...
  mutate(
    가정시가분류 = round((가정시가분류 + 가정시가수거) * 유량비율_가정시가분류, 2),
    가정시가수거 = round((가정시가분류 + 가정시가수거) * 유량비율_가정시가수거, 2),
    가정비시가분류 = round((가정비시가분류 + 가정비시가수거) * 유량비율_가정비시가분류, 2),
    가정비시가수거 = round((가정비시가분류 + 가정비시가수거) * 유량비율_가정비시가수거, 2),
    영업시가분류 = round((영업시가분류 + 영업시가수거) * 유량비율_영업시가분류, 2),
    영업시가수거 = round((영업시가분류 + 영업시가수거) * 유량비율_영업시가수거, 2),
    영업비시가분류 = round((영업비시가분류 + 영업비시가수거) * 유량비율_영업비시가분류, 2),
    영업비시가수거 = round((영업비시가분류 + 영업비시가수거) * 유량비율_영업비시가수거, 2),
    가정시가오수 = round((가정시가오수 + 가정시가단독) * 유량비율_가정시가오수, 2),
    가정시가단독 = round((가정시가오수 + 가정시가단독) * 유량비율_가정시가단독, 2),
    가정비시가오수 = round((가정비시가오수 + 가정비시가단독) * 유량비율_가정비시가오수, 2),
    가정비시가단독 = round((가정비시가오수 + 가정비시가단독) * 유량비율_가정비시가단독, 2),
    영업시가오수 = round((영업시가오수 + 영업시가단독) * 유량비율_영업시가오수, 2),
    영업시가단독 = round((영업시가오수 + 영업시가단독) * 유량비율_영업시가단독, 2),
    영업비시가오수 = round((영업비시가오수 + 영업비시가단독) * 유량비율_영업비시가오수, 2),
    영업비시가단독 = round((영업비시가오수 + 영업비시가단독) * 유량비율_영업비시가단독, 2)
  ) %>%
  select(-c(34:53))
...
P2_2.septictank <- septictank_water %>%
  ...
  mutate(
    설치연도 = 0,
    # 본번은 1~4까지 반복 작성
    본번 = rep(1:4, times = nrow(.) / 4),
    부번 = 0, 시설명 = "개인오수",
    건축물용도 = 2001, 시설용량 = 0, 청소여부 = 1, 용량적용 = "이하",
    하수처리시설명 = ifelse(방류형태 == "관로", 하수처리시설명, "")
  ) %>%
  select(
    ID, 연도, 설치연도, 행정구역코드, 시도, 시군구, 읍면동, 동리, 본번, 부번,
...
## 환경기초시설 현황 자료 불러오기
stp_1 <- read_excel("전국오염원조사/환경기초시설/2021년기준_전국오염원_조사자료_환경기초시설.xlsx",
  skip = 3, col_names = F
)

## 환경기초시설 추가 정리 자료 불러오기
stp_add <- read_excel("전국오염원조사/환경기초시설/전오사 환경기초시설 현황 정리.xlsx",
  skip = 1
) %>%
  select(2, 12:14)
...
## 환경기초시설 현황 자료 불러오기
## (엑셀파일에서 원주동화농공산업단지공공폐수처리시설 관거번호 2 → 1 수정)
## (원주동화농공산업단지공공폐수처리시설 수질 데이터가 없어 2020년 자료로 대체)
...
## 측정데이터 갯수 확인
stp_2_a <- stp_2_a %>%
  left_join(stp_2_a %>% count(처리시설명, 관거번호),
    by = c("처리시설명", "관거번호")
  )
...
# stp_2x <- stp_2_a %>% select(1, 2) %>% distinct()
# stp_2x <- S1.stp %>% select(처리시설명, 시군구) %>% distinct() %>%
#   left_join(S2.stp_pipe %>% select(처리시설명, 운영년도) %>% distinct(), by = "처리시설명")
#
#
#   left_join(stp_name %>% select(-시설코드), by = "처리시설명") %>%
#   left_join(stp_name %>% select(-처리시설명), by = "시설코드") %>%
...
stp_2_b <- stp_2_a %>%
  left_join(stp_2_a_mean, by = c("처리시설명", "관거번호")) %>%
  # 미측정 자료 평균값 입력
  mutate(
  ...
  # 관거가 여러개인 경우 유량은 합계, 농도는 유량가중평균농도 계산해서 관거 하나로 정리
  group_by(처리시설명, 운영월, 운영일) %>%
  mutate(
    관거이송농도BOD = sum(관거이송유량 * 관거이송농도BOD) / sum(관거이송유량),
    관거이송농도COD = sum(관거이송유량 * 관거이송농도COD) / sum(관거이송유량),
    관거이송농도TN = sum(관거이송유량 * 관거이송농도TN) / sum(관거이송유량),
    관거이송농도TP = sum(관거이송유량 * 관거이송농도TP) / sum(관거이송유량),
    관거이송유량 = sum(관거이송유량)
  ) %>%
  ungroup() %>%
  select(1, 3:12) %>%
  ...
  rowid_to_column(var = "ID") %>%
  mutate_at(vars(관거이송유량:관거이송농도TP), ~ replace(., is.na(.), 0)) %>%
...
## 측정데이터 갯수 확인
stp_3_a <- stp_3_a %>%
  left_join(stp_3_a %>% count(처리시설명, 유입원),
    by = c("처리시설명", "유입원")
  )

## 365일 자료가 없는 시설은 365일로 확장
stp_3_a_date <- stp_3_a %>%
  filter(n != 365) %>%
  select(처리시설명, 시설코드, 유입원, 운영년도) %>%
  distinct() %>%
  left_join(date, by = "운영년도") %>%
  left_join(stp_3_a,
    by = c(
      "처리시설명", "시설코드", "유입원", "운영년도", "운영월",
      "운영일"
    )
  )
...
S3.stp_direct <- stp_3_a %>%
  left_join(stp_3_a_mean, by = c("처리시설명", "유입원")) %>%
  mutate(
  ...
  select(1, 3:12) %>%
  # 유입원이 "기타"인 경우 제외(태백위생처리장)
  filter(유입원 != "기타") %>%
  # 유입원 항목에 띄어쓰기 제거
  mutate(유입원 = str_remove_all(유입원, "[:blank:]")) %>%
  rowid_to_column(var = "ID") %>%
  mutate_at(vars(ID), as.character)
...
stp_4_a <- stp_4 %>%
  ...
  relocate(c(운영년도, 운영월), .after = 유입구번호)
...
## 측정데이터 갯수 확인
stp_4_a <- stp_4_a %>%
  left_join(stp_4_a %>% count(처리시설명, 유입구번호),
    by = c("처리시설명", "유입구번호")
  )

## 365일 자료가 없는 시설은 365일로 확장
stp_4_a_date <- stp_4_a %>%
  filter(n != 365) %>%
  select(처리시설명, 시설코드, 유입구번호, 운영년도) %>%
  distinct() %>%
  left_join(date, by = "운영년도") %>%
  left_join(stp_4_a,
    by = c(
...
stp_4_a_mean <- stp_4_a %>%
  group_by(처리시설명, 유입구번호) %>%
  summarise_at(vars(총유입유량:총유입농도TP), list(mean = ~ mean(., na.rm = T)))

## 유입유량 및 농도 미측정시 평균값으로 입력
S4.stp_inflow <- stp_4_a %>%
  left_join(stp_4_a_mean, by = c("처리시설명", "유입구번호")) %>%
  mutate(
  ...
  group_by(처리시설명, 운영월, 운영일) %>%
  mutate(
    총유입농도BOD = sum(총유입유량 * 총유입농도BOD) / sum(총유입유량),
    총유입농도COD = sum(총유입유량 * 총유입농도COD) / sum(총유입유량),
    총유입농도TN = sum(총유입유량 * 총유입농도TN) / sum(총유입유량),
    총유입농도TP = sum(총유입유량 * 총유입농도TP) / sum(총유입유량),
    총유입유량 = sum(총유입유량)
  ) %>%
  ungroup() %>%
  filter(유입구번호 == 1) %>%
...
## 측정데이터 갯수 확인
stp_5_a <- stp_5_a %>%
  left_join(stp_5_a %>% count(처리시설명, 방류구번호),
    by = c("처리시설명", "방류구번호")
  )

## 365일 자료가 없는 시설은 365일로 확장
stp_5_a_date <- stp_5_a %>%
  filter(n != 365) %>%
...
## 자료 정리
industry1 <- industry %>%
  mutate_at(vars(52:76, 78:196), as.numeric) %>%
  mutate_at(vars(52:76, 78:196), ~ replace(., is.na(.), 0)) %>%
  mutate(
    년도 = 2021,
  ...
  set_names(
    c(
      ...
      "처리유형_조사", "종말처리", "위탁처리", "배출허용기준적용지역",
      "배출허용기준적용지역_조사"
    )
  ) %>%
  # 행정구역 코드(시군 직제순) 및 업소명 순서로 정렬
  arrange(행정구역코드, 업소명) %>%
  rowid_to_column(var = "일련번호")
...
P5.landuse <- landuse %>%
  ...
  mutate_at(vars(총면적:잡종지, 차집비), ~ replace(., is.na(.), 0)) %>%
...
landfill_a1 <- landfill_a %>%
  ...
  mutate(
    ...
    # 처리유형이 "처리후방류"인 경우 차집방법은 비워준다.
    # 차집방법이 "관로이송"인 경우 "관거이송"으로 변경
    차집방법 = case_when(
      처리유형 == "처리후방류" ~ as.character(NA),
      차집방법 == "관로이송" ~ "관거이송",
      TRUE ~ 차집방법
    )
  )
...
FP1.population <- P1.population
FP2_0.waterusage <- P2_0.waterusage
FP2_1.individual_STP <- P2_1.individual_STP
FP2_2.septictank <- P2_2.septictank

FP4.livestock <- data.frame()
FP4.livestock <- FP4.livestock %>%
  mutate(
    ID = 1, 년도 = 0, 행정구역코드 = "", 시도 = "", 시군구 = "",
    읍면동 = "", 리 = "", 젖소 = 0, 한우 = 0,
