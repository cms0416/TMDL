...
obs <- obs %>% mutate(월 = month(obs$일자)) %>%      # 월 추가
  ...
  mutate(BOD_측정부하량 = 유량 * BOD * 86.4) %>%     # 측정부하량 계산 
...
# 유량구간 및 목표수질 초과여부 확인
obs1 <- obs0 %>% left_join(target, by = "총량지점명") %>% 
...
ggsave(filename = "LDC graph_TP.jpg", path ="output/plot", scale = 1.15, width = 18, height = 11, dpi = 320, units = "cm")
...
#dev.off()
ggsave(filename = "LDC graph_TP(계절별).jpg", path ="output/plot", scale = 1.15, width = 18, height = 11, dpi = 320, units = "cm")
...
ggsave(filename = "LDC graph_TP(월별).jpg", path ="output/plot", scale = 1.15, width = 18, height = 11, dpi = 320, units = "cm")
...
total <- total %>% mutate(BOD초과율 = round(total$BOD_초과수/total$전체_자료수*100, 2),
                          TP초과율 = round(total$TP_초과수/total$전체_자료수*100, 2))

# 계절별 수질 현황
seasonal <- obs1 %>% group_by(계절) %>% 
  summarise(전체_자료수 = sum(자료수),
            BOD_초과수 = sum(BOD초과), 
            TP_초과수 = sum(TP초과))


seasonal <- seasonal %>% mutate(BOD초과율 = round(seasonal$BOD_초과수/seasonal$전체_자료수*100, 2),
                                BOD초과비율 = round(prop.table(seasonal$BOD_초과수)*100, 2),
                                TP초과율 = round(seasonal$TP_초과수/seasonal$전체_자료수*100, 2),
                                TP초과비율 = round(prop.table(seasonal$TP_초과수)*100, 2))

# 월별 수질 현황
monthly <- obs1 %>% group_by(월) %>% 
  summarise(전체_자료수 = sum(자료수),
            BOD_초과수 = sum(BOD초과), 
            TP_초과수 = sum(TP초과))

monthly <- monthly %>% mutate(BOD초과율 = round(monthly$BOD_초과수/monthly$전체_자료수*100, 2),
                              BOD초과비율 = round(prop.table(monthly$BOD_초과수)*100, 2),
                              TP초과율 = round(monthly$TP_초과수/monthly$전체_자료수*100, 2),
                              TP초과비율 = round(prop.table(monthly$TP_초과수)*100, 2)) 

# 수질자료 내보내기
write.csv(obs1, file = "D:/1. 수질총량/3. 목표수질/1. 한강수계/검토자료/LDC/월별계절별수질_TP.csv")