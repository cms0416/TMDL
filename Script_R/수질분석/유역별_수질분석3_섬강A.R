...
그래프_강수수질_상 <- 총량측정망_기상 %>%
  ...
  filter(연도 >= 2020) %>%
  ggplot() +
  geom_bar(aes(x = 일자, y = 일강수량 / 700, color = "steelblue2"),
           stat = "identity", width = 0.3, fill = "steelblue2"
  ) +
  geom_point(aes(x = 일자, y = TP, color = "black", fill = 계절),
             shape = 21, alpha = 0.6, size = 2.5
  ...
  scale_y_continuous(
    name = NULL,
    breaks = seq(0, 1, 0.1),
    sec.axis = sec_axis(~ . * 700, name = NULL, 
                        breaks = seq(0, 300, 20),
                        labels = scales::comma)
  ) +
  coord_cartesian(ylim = c(0.65, 0.8)) + # 데이터 자르지 않고 보이는 영역만 조정
  theme_bw(base_family = "notosanskr", base_size = 14) +
  scale_fill_discrete(name = "계절", na.translate = F) + # 범례 제목(NA값 제외)
  scale_color_manual(
    name = NULL, values = c("black", "red", "steelblue2"),
...
그래프_강수수질_하 <- 총량측정망_기상 %>%
  ...
  scale_y_continuous(
    name = "T-P(mg/L)",
    breaks = seq(0, 1, 0.1),
    sec.axis = sec_axis(~ . * 700, name = "강수량(mm)", labels = scales::comma)
  ) +
  scale_x_date(
    name = "연도",
    date_breaks = "1 year", 
  ...
  coord_cartesian(ylim = c(0, 0.3)) + # 데이터 자르지 않고 보이는 영역만 조정
  theme_bw(base_family = "notosanskr", base_size = 14) +
  scale_fill_discrete(name = "계절", na.translate = F) + # 범례 제목(NA값 제외)
  scale_color_manual(
    name = NULL, values = c("black", "red", "steelblue2"),
...
그래프_유량수질_상 <- 총량측정망 %>%
  ...
  coord_cartesian(ylim = c(0.65, 0.8)) +  # 데이터 자르지 않고 보이는 영역만 조정
  annotate(geom = "text", x = 5,  y = 0.75, label = "홍수기") +
  annotate(geom = "text", x = 25, y = 0.75, label = "풍수기") +
  annotate(geom = "text", x = 50, y = 0.75, label = "평수기") +
  annotate(geom = "text", x = 75, y = 0.75, label = "저수기") +
  annotate(geom = "text", x = 95, y = 0.75, label = "갈수기") +
  theme_test(base_family = "notosanskr", base_size = 14) +
  scale_fill_discrete(name = "계절") +  # 범례 제목
  scale_color_manual(
    name = NULL, values = c("black", "red", "steelblue2"),
...
## 월별 수질 박스플롯  ----------------------------------------------
그래프_월별_수질_박스플롯 <- 총량측정망 %>%
  ggplot(aes(x = factor(월), y = TP)) +
  # staplewidth : 박스플롯이 수염이 끝나는 지점 표시 너비
  geom_boxplot(outliers = FALSE, staplewidth = 0.3) +
  geom_jitter(aes(color = "black", fill = 계절),
              shape = 21, alpha = 0.6, size = 2.3,
...
## ★ 그래프 pdf 출력  ----------------------------------------------------------
pdf("E:/Coding/TMDL/수질분석/Output/Plot/그래프_섬강A_TP(8.4x5).pdf",
    width = 8.4, height = 5
)
...
총량측정망_기상 %>%
  ...
  ggplot() +
  geom_bar(aes(x = 강우크기순서, y = 일강수량 / 300, color = "steelblue2"),
           stat = "identity", width = 0.3, fill = "steelblue2"
  ) +
...
수질측정망 %>%
  ...
  scale_x_discrete(limits = c("섬강A1", "섬강A2", "섬강A3", "섬강A")) +
  scale_y_log10(name = "T-P(mg/L)") +
  scale_color_manual(
    name = NULL, values = c("black", "red"),
    labels = c("수질", "목표수질")
...
## 연도 별 유량 박스플롯  ----------------------------------------------
그래프_연도별_유량_박스플롯 <- 총량측정망 %>%
  ggplot(aes(x = factor(연도), y = 유량)) +
  # staplewidth : 박스플롯이 수염이 끝나는 지점 표시 너비
  geom_boxplot(outliers = FALSE, staplewidth = 0.3) +
  geom_jitter(aes(color = "black", fill = 계절),
              shape = 21, alpha = 0.6, size = 2.3,
  ...
  scale_x_discrete(name = "연도") +
  scale_y_log10(name = "유량(㎥/S)") +
  scale_color_manual(
    name = NULL, values = c("black"),
    labels = c("유량")
  ) +
  theme_bw(base_family = "notosanskr", base_size = 14) +
  theme(
