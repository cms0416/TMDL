...
## 기상자료
기상자료 <- read_excel("수질분석/기상자료/평창기상대_강수량_기온_2014_2023.xlsx") %>%
  select(-c(지점, 지점명)) %>%
...
강수량_월별 <- 기상자료 %>%
  ...
  # 월별 강수량 평균
  group_by(월) %>%
  summarise(월강수량_평균 = mean(월강수량, na.rm = TRUE), .groups = "drop")
...
original_plot <- 총량측정망 %>%
  ...
  annotate(geom = "text", x = 5, y = 1.15, label = "홍수기") +
  annotate(geom = "text", x = 25, y = 1.15, label = "풍수기") +
  annotate(geom = "text", x = 50, y = 1.15, label = "평수기") +
  annotate(geom = "text", x = 75, y = 1.15, label = "저수기") +
  annotate(geom = "text", x = 95, y = 1.15, label = "갈수기") +
...
p1 <- 총량측정망 %>%
  ...
  coord_cartesian(ylim = c(0.33, 1.2)) +  # 데이터 자르지 않고 보이는 영역만 조정
  annotate(geom = "text", x = 5, y = 1.15, label = "홍수기") +
  annotate(geom = "text", x = 25, y = 1.15, label = "풍수기") +
  annotate(geom = "text", x = 50, y = 1.15, label = "평수기") +
  annotate(geom = "text", x = 75, y = 1.15, label = "저수기") +
  ...
  theme(
    ...
    plot.margin = margin(5, 20, 5, 5)  # 우측 여백 추가
  )
...
p2 <- 총량측정망 %>%
  ...
  coord_cartesian(ylim = c(0, 0.21)) +  # 데이터 자르지 않고 보이는 영역만 조정
  theme_test(base_family = "notosanskr", base_size = 14) +
  scale_color_manual(
    name = NULL, values = c("black", "red", "steelblue2"),
    labels = c("수질", "목표수질", "유량")
...
## 그래프 pdf 출력
pdf("E:/Coding/TMDL/수질분석/Output/Plot/그래프_평창A_TP(8.4x5).pdf",
  width = 8.4, height = 5)
...
총량측정망 %>%
  ggplot() +
  geom_vline(xintercept = c(0, 10, 40, 60, 90, 100), linetype = "dashed", linewidth = 0.5) +
  geom_point(aes(
...
수질측정망 %>%
  ...
  scale_x_discrete(limits = c("평창A1", "평창A2", "평창A")) +
