...
법정동코드 <- read_excel("주소 검토/법정동코드_강원_240619.xlsx") %>%
  ...
  select(-c(폐지여부, 시도, 리)) %>% 
  set_names(c("pnu_code", "sigun", "emd", "dongli"))

## 지목
지목 <- read_excel("D:/GIS/토지이용/연속지적도/지목부호.xlsx") %>% 
  set_names(c("jimok_abb", "jimok"))
...
연속지적도_정리 <- 연속지적도 %>%
  ...
  # st_drop_geometry() %>% 
  # rename(단위유역 = SW_NAME) %>%
  filter(면적 != 0, SW_NAME == "섬강A") %>%
  mutate(
    pnu_code = str_sub(PNU, 1, 10) %>% as.numeric()
  ) %>%
  left_join(법정동코드, by = "pnu_code") %>%
  mutate(
    jibun_1 = str_sub(JIBUN, 1, -2) %>% str_trim(),
    jimok_abb = str_sub(JIBUN, -1),
    .after = dongli
  ) %>%
  mutate(
    sigun = ifelse(is.na(PNU), 시군구명, sigun),
    emd = ifelse(is.na(PNU), 읍면동명, emd),
    dongli = ifelse(is.na(PNU), 동리명, dongli),
    jimok_abb = ifelse(is.na(PNU), "임", jimok_abb)
  ) %>% 
  left_join(지목, by = "jimok_abb") %>% 
  mutate(jimok = ifelse(jimok_abb == "가", "가", jimok)) %>% 
  # 지적도와 동리 자료를 합치면서 경계 불일치로 필지가 분할된 경우 다시 합치기
  group_by(PNU, sigun, emd, dongli, jibun_1, jimok) %>% 
  summarise(area = sum(면적, na.rm=TRUE) / 10^6, .groups = "drop")
...
섬강A_농지 <- 연속지적도 %>% 
  ...
  # set_names(c("pnu", "sigun", "emd", "dongli", "jibun", "jimok", "area", "geometry"))
