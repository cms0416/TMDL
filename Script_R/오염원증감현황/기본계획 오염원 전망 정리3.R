...
## 기본계획 오염원 전망 자료 불러오기
prospect <- read_excel("오염원증감현황/오염원 전망(기본계획).xlsx")
...
prospect_s2 <- prospect_s %>%
  ...
  select(시군구, 단위유역, 오염원, 분류, `2017년`, `2020년`, `2025년`, `2030년`, `2022년`) %>% 
  filter(시군구 != "강원도" | 단위유역 != "소계") %>% 
...
###########################  전망자료와 당해년도 자료 합치기  ##########################
present <- read_excel("전국오염원조사/Output/전국오염원조사 자료 정리(총량대상시군기준).xlsx") %>% 
  select(시군구:분류, `2022`)

data_join <- present %>% 
  left_join(prospect_s2%>% select(시군구:분류, `2022년`, `2030년`), 
            by = c("시군구", "단위유역", "오염원", "분류"))
