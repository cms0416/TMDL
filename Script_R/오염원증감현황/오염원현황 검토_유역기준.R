...
pol <- pol %>%
  ...
  group_by(대상물질, 단위유역, 시군, 오염원, 점비점) %>%
  summarise(부하량 = sum(부하량)) %>%
  setnames(., "오염원", "구분")

## 오염원별 배출부하량 합계 계산
pol_sum <- pol %>% 
  group_by(대상물질, 단위유역, 시군, 점비점) %>% 
  summarise(합계 = sum(부하량))

## 잔여량 계산(할당오염원 - 오염원별 배출부하량 합계)
jan <- data %>% filter(구분 == "할당오염원") %>% 
  left_join(pol_sum, by = c("대상물질", "단위유역", "시군", "점비점")) %>% 
...
## 최종 자료 정리
total <- total %>%
  mutate_at(vars(단위유역, 시군, 구분, 점비점), as.factor) %>%
  mutate(
  ...
  arrange(대상물질, 단위유역, 시군, 점비점, 구분)


### 파일 내보내기
write_xlsx(total, path = "Output/Data/오염원현황 정리_유역기준.xlsx")
