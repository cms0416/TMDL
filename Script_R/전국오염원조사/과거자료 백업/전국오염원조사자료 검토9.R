...
#   filter(시군 == "강원도") %>%
#   group_by(권역) %>%
#   summarise_at(vars(`2014`:`2023`), ~ sum(.)) %>%
...
##### ===== 계별 대표값 정리 ===================================================
## 축산계 주요 축종(젖소 + 한우 + 돼지 / 가금)
축산계_4합계_2 <- 축산계_4합계_1 %>%
  filter(분류 %in% c("젖소", "한우", "돼지")) %>%
  group_by(단위유역, 시군, 오염원) %>%
  summarise_at(vars(`2014`:`2023`), ~ sum(.)) %>%
  mutate(분류 = "소, 돼지") %>% 
  bind_rows(축산계_4합계_1 %>% filter(분류 == "가금"))
...
## 계별 대표값 합친 후 정리
계별대표값 <- bind_rows(
  생활계_인구_4합계_1, 생활계_물사용량_4합계_1, 축산계_4합계_2, 
  산업계_4합계_폐수방류_2, 토지계_4합계_2
)

계별대표값 %<>% 
  mutate(
    분류 = factor(분류, levels = c(
      "인구", "물사용량", "소, 돼지", "가금", "산업폐수", "농경지"
    ))
  ) %>%
  arrange(단위유역, 시군, 분류)
