...
# 본 코드는 수질개선사업계획 추진실적 뿐만 아니라 전국오염원조사 자료 검토를 
# 위해 만들어 졌기 때문에 권역별 정리 등 추진실적과 관계 없는 내용들도 일부 
# 포함 되어 있으며, 21년 자료만이 아닌 14년 자료부터 정리되도록 되어 있습니다.
# 참고하시기 바랍니다.
...
# 반올림 사용자 정의 함수 로드(R 기본 반올림 방식인 Banker’s Rounding이 아닌 사사오입 방식 적용)
...
# 경로지정된 생활계 파일 합치기
population <- files %>%
  map_dfr(read_excel, skip = 4, col_names = F) %>% # map_dfr : 행 병합(row-binding)하여 작성된 데이터프레임 반환
  select(-c(7:18)) # 필요없는 열 삭제
## *****************************************************************************


# 수치데이터 및 연도 숫자로 지정
population <- population %>% mutate_at(vars(1, 7:23), as.numeric)
...
population_sum_a <- population_sum %>%
  filter(시군구 == "합계") %>%
  group_by(권역) %>%
  summarise_at(vars(c("2014":"2021")), funs(sum)) %>%
  # 여러 변수(vars)에 복수의 통계치(funs) 계산
...
# 경로지정된 생활계 파일 합치기
waterusage <- files %>%
  map_dfr(read_excel, sheet = 2, skip = 4, col_names = F) %>% # map_dfr : 행 병합(row-binding)하여 작성된 데이터프레임 반환
...
waterusage <- waterusage %>% mutate_at(vars(연도, 가정용물사용합계, 영업용물사용합계), as.numeric) %>% # 수치데이터 및 연도 숫자로 지정
  mutate(
    ...
    주소 = paste(시군구, 법정동, ifelse(is.na(waterusage$법정리), 법정동, 법정리)) # 주소코드 추가("리"가 없는 "동"의 경우 "법정리"칸에 "동"으로 추가)
  )
...
for (i in 2014:2021) {
  temp <- share_waterusage %>%
    ...
    mutate(물사용량 = round2(생활계 * 물사용량, 1)) %>%  # 동리별 물사용량합계와 유역 점유율 계산
    group_by(연도, 단위유역, 시군구) %>%
...
waterusage_sum_a <- waterusage_sum %>%
  filter(시군구 == "합계") %>%
  group_by(권역) %>%
  summarise_at(vars(c("2014":"2021")), funs(sum)) %>% # 여러 변수(vars)에 복수의 통계치(funs) 계산
  mutate(권역 = factor(권역, levels = c(
    "남한강", "섬강", "북한강", "홍천강", "한탄강", "충청북도", "경기도", "낙동강", "기타"
  ))) %>%
  arrange(권역)
...
for (file in files) {
  print(file)
  temp <- read_excel(paste(dir, file, sep = "/"), skip = 6, col_names = F) %>%
    select(7, 8, 9, 12, 13) %>%
    mutate(연도 = substr(file, 1, 4)) # substr(문자열, 시작위치, 끝위치) : 지정한 문자열(file)의 첫번째 부터 네번째 까지 문자열(연도) 추출
...
# 사육두수, 연도 숫자로 지정
livestock <- livestock %>% mutate_at(vars(사육두수, 연도), as.numeric)

# 주소코드 추가("리"가 없는 "동"의 경우 "법정리"칸에 "동"으로 추가)
# 축산계 축종 변환(총량 기술지침 발생원단위 기준 축종명으로 변경)
livestock <- livestock %>%
  mutate(주소 = paste(시군구, 법정동, ifelse(is.na(livestock$법정리), 법정동, 법정리))) %>%
  mutate(축종 = ifelse(축종 == "한우(소)", "한우",
    ifelse(축종 == "유우(젖소)", "젖소",
      ifelse(축종 == "돼지", "돼지",
        ifelse(축종 == "마필(말)", "말",
          ifelse(축종 == "산양(염소포함)" | 축종 == "면양(육양포함)" | 축종 == "사슴", "양, 사슴",
            ifelse(축종 == "개", "개",
              ifelse(축종 == "닭" | 축종 == "오리" | 축종 == "타조" | 축종 == "가금기타", "가금", "-")
            )
          )
        )
      )
...
for (i in 2014:2021) {
  temp <- share_livestock %>%
    ...
    mutate(총사육두수 = round(축산계 * 사육두수)) %>% # 동리별 사육두수합계와 유역 점유율 계산
    group_by(연도, 단위유역, 시군구, 축종) %>%
    summarise(총사육두수 = sum(총사육두수, na.rm = T))
...
# 권역 추가 후 단위유역, 시군구, 축종, 권역 factor로 지정
livestock_sum <- livestock_sum %>%
  ...
  select(권역, everything()) %>%
  mutate_at(vars(단위유역, 시군구, 축종, 권역), as.factor)
...
#####  엑셀 파일 내보내기_writexl
write_xlsx(list("유역별정리" = livestock_sum, "시군별정리" = livestock_sum_s, "권역별정리" = livestock_sum_a), path = "Output/Data/전국오염원조사/축산계.xlsx")
...
for (file in files) {
  print(file)
  temp <- read_excel(paste(dir, file, sep = "/"), skip = 5, col_names = F) %>%
...
# 변수명 지정
names(industry) <- c(
  "휴업", "업소명", "시군구", "법정동", "법정리", "규모", "폐수발생량", "폐수방류량", "연도"
)

# 휴업인 경우 삭제
industry <- industry %>% filter(is.na(휴업))

# 폐수발생량, 폐수방류량, 연도 숫자로 지정(mutate_at : 여러 변수(vars)에 복수의 통계치(funs) 계산 후 새로운 변수로 추가)
industry <- industry %>% mutate_at(vars(폐수발생량, 폐수방류량, 연도), as.numeric)
...
industry_summary1 <- industry_total %>%
  group_by(연도, 시군구) %>%
  summarise(폐수방류량 = sum(폐수방류량))

# 각 동리별 단위유역 점유율 계산(소유역 점유율 합계)
share_industry <- share %>%
  group_by(주소, 단위유역, 시군구) %>%
  summarise(산업계 = sum(산업계) / 100) %>%
  mutate(
...
for (i in 2014:2021) {
  temp <- share_industry %>%
    ...
    mutate(
      업소수 = round(산업계 * 업소수), # 동리별 업소수합계와 유역 점유율 계산
      폐수발생량 = round2(산업계 * 폐수발생량, 1), # 동리별 폐수발생량합계와 유역 점유율 계산
      폐수방류량 = round2(산업계 * 폐수방류량, 1) # 동리별 폐수방류량합계와 유역 점유율 계산
    ) %>%
...
#####  엑셀 파일 내보내기_writexl
sheets <- list(
  "산업계_업소수_유역별" = industry_sum1, "산업계_업소수_시군별" = industry_sum1_s, "산업계_업소수_권역별" = industry_sum1_a,
  "산업계_폐수발생량_유역별" = industry_sum2, "산업계_폐수발생량_시군별" = industry_sum2_s, "산업계_폐수발생량_권역별" = industry_sum2_a,
  "산업계_폐수방류량_유역별" = industry_sum3, "산업계_폐수방류량_시군별" = industry_sum3_s, "산업계_폐수방류량_권역별" = industry_sum3_a
)

write_xlsx(sheets, path = "Output/Data/전국오염원조사/산업계.xlsx")
...
# 법정동 변수명 바꾸기
setnames(landuse, "법정동(읍면)", "법정동")

# 지목별 면적 및 연도 숫자로 지정
landuse <- landuse %>% mutate_at(vars(총면적:연도), as.numeric)


# 주소코드 추가("리"가 없는 "동"의 경우 "법정리"칸에 "동"으로 추가)
# 토지계 지목 변환(총량 기술지침 발생원단위 지목에 맞춰서 조정)
landuse <- landuse %>%
  mutate(주소 = paste(시군구, 법정동, ifelse(is.na(landuse$법정리), 법정동, 법정리))) %>%
  mutate(
    기타초지 = 목장용지 + 공원 + 묘지 + 사적지,
...
for (i in 2014:2021) {
  temp <- share_landuse %>%
    ...
    summarise(총면적 = round2(sum(총면적, na.rm = T) * 10^-6, 3))
...
# 권역 추가 후 단위유역, 시군구, 지목, 권역 factor로 지정
landuse_sum <- landuse_sum %>%
  ...
  select(권역, everything()) %>%
  mutate_at(vars(단위유역, 시군구, 지목, 권역), as.factor) 
...
#####  엑셀 파일 내보내기_writexl
write_xlsx(list("유역별정리" = landuse_sum, "시군별정리" = landuse_sum_s, "권역별정리" = landuse_sum_a), path = "Output/Data/전국오염원조사/토지계.xlsx")
...
for (file in files) {
  print(file)
  temp <- read_excel(paste(dir, file, sep = "/"), skip = 3, col_names = F) %>%
...
# 변수명 지정
names(fishfarm) <- c(
  "업소명", "시군구", "법정동", "법정리", "종류", "시설면적", "방류하천", "휴업", "연도"
)
...
# 시설면적, 연도 숫자로 지정
fishfarm <- fishfarm %>% mutate_at(vars(시설면적, 연도), as.numeric)

# 연도 및 동리별 시설면적 합계 정리
fishfarm_total <- fishfarm %>%
  group_by(연도, 주소, 시군구, 종류) %>%
  summarise(시설면적 = sum(시설면적))

# 각 동리별 단위유역 점유율 계산(소유역 점유율 합계)
share_fishfarm <- share %>%
  group_by(주소, 단위유역, 시군구) %>%
  summarise(양식계 = sum(양식계) / 100) %>%
  mutate(
    가두리 = "가두리", 지수식 = "지수식", 유수식 = "유수식", 대하 = "대하"
  ) %>%
  pivot_longer(cols = 가두리:대하, names_to = "종류", values_to = "임시") %>%
  select(-임시)
...
for (i in 2014:2021) {
  temp <- share_fishfarm %>%
    ...
    mutate(시설면적 = round2(양식계 * 시설면적, 2)) %>% # 동리별 시설면적 합계와 유역 점유율 계산
    group_by(연도, 단위유역, 시군구, 종류) %>%
    summarise(시설면적 = sum(시설면적, na.rm = T))
...
# 권역 추가 후 단위유역, 시군구, 종류, 권역 factor로 지정
fishfarm_sum <- fishfarm_sum %>%
  ...
  select(권역, everything()) %>%
  mutate_at(vars(단위유역, 시군구, 종류, 권역), as.factor)
...
#####  엑셀 파일 내보내기_writexl
write_xlsx(list("유역별정리" = fishfarm_sum, "시군별정리" = fishfarm_sum_s, "권역별정리" = fishfarm_sum_a), path = "Output/Data/전국오염원조사/양식계.xlsx")
...
for (file in files) {
  print(file)
  temp <- read_excel(paste(dir, file, sep = "/"), sheet = 2, skip = 2, col_names = F) %>%
    select(1, 4) %>%
    mutate(연도 = substr(file, 1, 4)) # substr(문자열, 시작위치, 끝위치) : 지정한 문자열(file)의 첫번째 부터 네번째 까지 문자열(연도) 추출
  landfill <- rbind(landfill, temp)
}
## *****************************************************************************
...
# 침출수 발생유량 연평균 계산
landfill_mean <- landfill %>%
  mutate_at(vars(연도, 발생유량), as.numeric) %>%
  group_by(매립시설명, 연도) %>%
  summarise(발생유량 = round2(mean(발생유량), 1)) %>%
  mutate_all(~ replace(., is.na(.), 0)) %>%
  mutate(시설수 = 1)

# 매립장 현황 데이터 불러오기
landfill_a <- read_excel("Data/전국오염원조사/매립장 현황.xlsx") %>%
...
for (i in 2014:2021) {
  temp <- share %>%
    ...
    group_by(연도, 단위유역, 시군구) %>%
    summarise(시설수 = sum(시설수))

  landfill_count <- rbind(landfill_count, temp)
}

# 결측치 0으로 변환
landfill_count <- landfill_count %>% mutate_all(~ replace(., is.na(.), 0))
...
alldata_s <- alldata_s %>%
  ...
  mutate(단위유역 = factor(단위유역, levels = c(
    ...
    "북한A", "북한B", "소양A", "인북A", "소양B", "북한C", "홍천A", "한탄A",
    "한강B", "제천A", "한강D", "북한D", "한탄B", "임진A", "낙본A", "기타"
  ))) %>%
  mutate(시군구 = factor(시군구, levels = c(
    "춘천시", "원주시", "강릉시", "태백시", "삼척시", "홍천군",
    "횡성군", "영월군", "평창군", "정선군", "철원군", "화천군",
  ...
  arrange(시군구, 단위유역, 오염원)

## 수질개선사업계획 추진실적 기준으로 정리(기타수계 및 타시도 관할 제외)
...
###  엑셀 파일 내보내기_writexl
write_xlsx(alldata_s1, path = "Output/Data/전국오염원조사/전국오염원조사 자료 정리(수질개선사업추진실적기준).xlsx")
