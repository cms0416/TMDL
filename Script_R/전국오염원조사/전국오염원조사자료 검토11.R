...
##### 공통파일(단위유역별 점유율) 불러오기  ####################################
동리유역현황 <- read_excel("전국오염원조사/동리_유역_현황.xlsx") %>% 
  mutate(동리코드 = str_c(시군, 읍면동, 동리, sep = " ")) %>% 
  select(동리코드, 단위유역, 점유율)

share_원본 <- read_excel("전국오염원조사/단위유역별 점유율.xlsx")
  
share <- share_원본 %>% 
  group_by(동리코드, 단위유역, 시군) %>%
  summarise(across(생활계:유원지, ~ sum(.x) / 100), .groups = "drop") %>%
  left_join(동리유역현황, by = c("동리코드", "단위유역")) %>%
  mutate(
    across(
      생활계:유원지,
      ~ ifelse(점유율 == 100 & .x < 1, 1, .x)
    )
  )
...
철원군 <- 데이터통합_동리 %>% 
  filter(시군 == "철원군",
         !단위유역 %in% c("기타", "소계"),
         동리 %in% c("화지리", "사요리", "외촌리", "율이리", "내포리", "대마리", 
  ...
  relocate(동리, .after = 분류) %>% 
  mutate(across(c(임진A:한탄A), ~replace(., is.na(.), ""))) %>% 
  mutate(동리 = factor(동리, levels = c(
