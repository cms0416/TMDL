...
data_내부검토 <- data %>%
  ...
  # 필요한 열 선택
  select(
    1:5, 9:12, 64, 65, 70, 71, 88, 140, 141, 146, 147, 167, 168, 173, 삭감방법
  ) %>%
  # 열 이름 설정
  set_names(c(
...
##### 전년도까지 소진량 정리 ##### ---------------------------------------------
내부_전년도까지 <- data_내부검토 %>%
  filter(관리자번호 != "기본계획_기승인", 협의연도 < 2024) %>%
  load_cal(.) %>%
  relocate(대상물질) %>%
...
## 데이터 정리
협의사업목록 <- data_내부검토 %>% 
  select(관리자번호:단위유역, 사업명, 준공여부, 협의상태, 협의연도, 협의일자, 사업종류) %>% 
