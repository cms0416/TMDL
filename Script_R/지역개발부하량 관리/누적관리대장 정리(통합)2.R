...
data1 <- data %>%
  ...
  mutate(
    할당연도 = year(할당일자),
    협의연도 = year(협의일자),
    # 최초 및 기승인 개발은 협의연도를 2020년으로 설정, 협의일자가 누락된 경우 할당연도로 설정
    협의연도 = case_when(
...
result2 %<>%
  ...
  # 소진율 추가
  mutate(
    소진율.전체_점 = round(소진량_점 / 지역개발_점, 3) * 100,
    소진율.전체_비점 = round(소진량_비점 / 지역개발_비점, 3) * 100,
    소진율.신규_점 = round(신규_점 / (지역개발_점 - 기승인_점), 3) * 100,
    소진율.신규_비점 = round(신규_비점 / (지역개발_비점 - 기승인_비점), 3) * 100,
