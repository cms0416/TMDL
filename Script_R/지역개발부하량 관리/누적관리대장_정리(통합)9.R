...
data_내부검토 <- data %>%
  ...
  mutate(across(c(18, 23, 28, 33, 38, 43, 48, 53, 58), ~ replace(., is.na(.), "/"))) %>%
  # 삭감방법 열(BOD 삭감방법만 정리 : BOD, T-P 삭감방법 동일) 모두 합치고, NA 대체값 삭제
  mutate(
    삭감방법 =
      str_c(...13, ...18, ...23, ...28, ...33,
        ...38, ...43, ...48, ...53, ...58,
        sep = ", "
      ) %>%
        str_remove_all(., ", /") %>%
  ...
  # 필요한 열 선택
  select(
    1:5, 9:12, 64, 65, 70, 71, 88, 140, 141, 146, 147, 
    167, 168, 170:173, 삭감방법
  ) %>%
  # 열 이름 설정
  set_names(c(
    "사업구분", "시군", "단위유역", "할당일자", "사업명", "착공연도",
    "준공예정연도", "준공여부", "BOD_삭감량", "BOD_지역개발점", 
    "BOD_지역개발비점", "BOD_소진점", "BOD_소진비점", "TP_삭감량", 
    "TP_지역개발점", "TP_지역개발비점","TP_소진점", "TP_소진비점", 
  ...
  mutate(across(c(착공연도, 준공예정연도, BOD_삭감량:TP_소진비점), as.numeric)) %>%
...
추진실적_사업목록 <- data_추진실적 %>%
  select(
    사업구분:단위유역, 사업명, 협의일자, 준공예정연도,
    BOD_소진점, BOD_소진비점, TP_소진점, TP_소진비점,
    삭감방법, BOD_삭감량, TP_삭감량,
    할당일자, 착공연도, 준공여부, 협의상태, 협의연도
  ) %>%
  mutate(구분 = case_when(
    협의연도 > 2023 | 사업구분 == "기본계획_최초개발" ~ "제외",
    협의연도 < 2023 ~ "협의 부하량",
    협의연도 == 2023 ~ "사용 부하량"
  )) %>%
  order_func() %>%
  arrange(시군, 단위유역, 협의일자)
...
write_xlsx(추진실적_사업목록,
  path = "지역개발부하량 관리/Output/누적관리대장_사업목록(추진실적).xlsx"
)
...
##### 전년도까지 소진량 정리  --------------------------------------------------
내부_전년도까지 <- data_내부검토 %>%
  filter(사업구분 != "기본계획_기승인", 협의연도 < 2024) %>%
  load_cal(.) %>%
  relocate(대상물질) %>%
...
협의건수_func <- function(data) {
  result <- data %>%
    ...
    # 최초개발은 협의 현황에서 제외
    select(시군, 단위유역, 기승인, 신규, 
           기간외소진, 재협의, 사업취소) %>%
    # 재협의는 중복되므로 총사업건수에서 제외
    mutate(총사업건수 = 기승인 + 신규 + 기간외소진 - 사업취소, 
           .after = 단위유역) %>%
    # 강원도 총계 계산
...
미준공사업내역 <- data_내부검토 %>%
  ...
  filter(협의상태 == "협의", 준공여부 == "미준공", 준공예정연도 <= 2024) %>%
  order_func() %>%
...
내부_결과_그래프 %>% 
  ...
  scale_x_discrete(limits = c("춘천", "원주", "강릉", "태백(낙)", "태백(한)", 
                              "삼척", "홍천", "횡성", "영월", "평창", "정선",
                              "철원", "화천", "양구", "인제", "고성")) +
  scale_y_continuous(name = "BOD 점 지역개발부하량(kg/일)", 
...
내부_결과_그래프 %>% 
  ...
  scale_x_discrete(limits = c("춘천", "원주", "강릉", "태백(낙)", "태백(한)", 
                              "삼척", "홍천", "횡성", "영월", "평창", "정선",
                              "철원", "화천", "양구", "인제", "고성")) +
  scale_y_continuous(name = "BOD 비점 지역개발부하량(kg/일)", 
...
내부_결과_그래프 %>% 
  ...
  scale_x_discrete(limits = c("춘천", "원주", "강릉", "태백(낙)", "태백(한)", 
                              "삼척", "홍천", "횡성", "영월", "평창", "정선",
                              "철원", "화천", "양구", "인제", "고성")) +
  scale_y_continuous(name = "T-P 점 지역개발부하량(kg/일)", 
...
내부_결과_그래프 %>% 
  ...
  scale_x_discrete(limits = c("춘천", "원주", "강릉", "태백(낙)", "태백(한)", 
                              "삼척", "홍천", "횡성", "영월", "평창", "정선",
                              "철원", "화천", "양구", "인제", "고성")) +
  scale_y_continuous(name = "T-P 비점 지역개발부하량(kg/일)", 
