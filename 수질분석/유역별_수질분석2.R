...
기상지표_일별 <- 기상자료_시단위 %>%
  ...
  summarise(
    ...
    고강도강우_발생여부 = any(강수량 >= 20, na.rm = TRUE),
...
그래프_유량수질_상 <- 총량측정망 %>%
  ...
  scale_y_continuous(
    name = "",
    breaks = seq(0, 0.8, 0.1),
    sec.axis = sec_axis(~ . * 250, name = "", labels = scales::comma)
  ) +
  scale_x_continuous(
    name = NULL,
    breaks = seq(0, 100, 10), # x축 표시 형식 설정
  ...
  annotate(geom = "text", x = 5, y = 1.1, label = "홍수기") +
  annotate(geom = "text", x = 25, y = 1.1, label = "풍수기") +
  annotate(geom = "text", x = 50, y = 1.1, label = "평수기") +
  annotate(geom = "text", x = 75, y = 1.1, label = "저수기") +
  annotate(geom = "text", x = 95, y = 1.1, label = "갈수기") +
  theme_test(base_family = "notosanskr", base_size = 14) +
  scale_fill_discrete(name = "계절") + # 범례 제목
  scale_color_manual(
    name = NULL, values = c("black", "red", "steelblue2"),
...
# 두 그래프를 위아래로 배치
그래프_유량수질 <- p1 / p2 +
  plot_layout(heights = c(1, 2.3)) # 상하 비율 조정
