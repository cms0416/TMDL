...
## 유량 / 수질(T-P) 그래프  ----------------------------------------------------
pdf("E:/Coding/TMDL/수질분석/Output/Plot/그래프_한탄A_TP(8.4x5).pdf",
    width = 8.4, height = 5)
...
총량측정망 %>%
  ...
  annotate(geom = "text", x = 5,  y = 0.5, label = "홍수기") +
  annotate(geom = "text", x = 25, y = 0.5, label = "풍수기") +
  annotate(geom = "text", x = 50, y = 0.5, label = "평수기") +
  annotate(geom = "text", x = 75, y = 0.5, label = "저수기") +
  annotate(geom = "text", x = 95, y = 0.5, label = "갈수기") +
  annotate(geom = "text", x = 25, y = 0.31, label = "중권역 목표수질 0.04 mg/L", color="red") +
  geom_segment(aes(x = 19, y = 0.29, xend = 19, yend = 0.045), 
               arrow = arrow(angle=20, length=unit(.3,"cm")),
