...
landuse_sum <- landuse_sum %>% 
  ...
  mutate(차이 = replace(차이, is.na(차이), 0),
         총면적 = 총면적 + 차이) %>% 
  select(-차이)
## *****************************************************************************
...
share_fishfarm <- share %>%
  ...
  mutate(
    가두리 = "가두리", 유수식 = "유수식", 도전양식 = "도전양식", 지수식 = "지수식"
  ) %>%
...
###  엑셀 파일 내보내기_writexl
write_xlsx(alldata_s, path = "Output/Data/전국오염원조사/전국오염원조사 자료 정리(강원도전체시군기준).xlsx")
write_xlsx(alldata_s1, path = "Output/Data/전국오염원조사/전국오염원조사 자료 정리(총량대상시군기준).xlsx")

write.csv(alldata_s1, "Output/Data/전국오염원조사/전국오염원조사 자료 정리(총량대상시군기준).csv", row.names=F, fileEncoding = 'cp949')
