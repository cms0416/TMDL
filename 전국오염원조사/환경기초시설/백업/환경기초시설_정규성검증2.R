...
# Setting 
data %<>% 
    set_names(c("TWW", "처리시설코드", "운영일자", "번호", "Flow", "BOD", "TP"))
    


data <- read.csv('환경기초_2021.csv', header = TRUE)
data <- select(data, TWW, BOD) # 처리시설명과 BOD 수치만 선택

data <- na.omit(data)  # 결측값 제거
data <- data[!(data$BOD == 0),]  # 0값 제거
...
write.csv(result_cbind_KS_SW_AD,"전국오염원조사/기준배출수질/Output/Result_환경기초시설_정규성검증_2021_result_BOD.csv") #정규성 검정 BOD 결과
