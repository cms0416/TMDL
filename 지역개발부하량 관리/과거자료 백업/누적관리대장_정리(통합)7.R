...
data_내부검토 <- data %>%
  ...
  mutate(across(c(13, 18, 23, 28, 33, 38, 43, 48, 53, 58), ~ replace(., is.na(.), "/"))) %>%
  ...
  # 필요한 열 선택
  select(
    1:5, 9:12, 64, 65, 70, 71, 88, 140, 141, 146, 147, 167, 168, 170:173, 삭감방법
  ) %>%
  # 열 이름 설정
  set_names(c(
    "사업구분", "시군", "단위유역", "할당일자", "사업명", "착공연도",
    "준공예정연도", "준공여부", "BOD_삭감량", "BOD_지역개발점", "BOD_지역개발비점",
    "BOD_소진점", "BOD_소진비점", "TP_삭감량", "TP_지역개발점", "TP_지역개발비점",
    "TP_소진점", "TP_소진비점", "협의일자", "협의상태", "주관부서", "담당자명", 
    "부서연락처", "사업종류", "삭감방법"
  )) %>%
...
data %>% 
  ...
  단위유역 = factor(단위유역, levels = c(
    "합계", "소계", "골지A", "오대A", "주천A", "평창A", "옥동A", "한강A", "섬강A",
...
## 데이터 정리
협의사업목록 <- data_내부검토 %>% 
  select(사업구분:단위유역, 사업명, 준공예정연도, 준공여부, 협의상태, 협의연도, 
         협의일자, 사업종류) %>% 
  # filter(!(사업구분 != "사업취소" & 협의상태 == "사업 취소")) %>% 
  mutate(사업구분 = case_when(
    사업구분 == "기본계획_기승인" ~ "기승인",
    사업구분 == "기본계획_최초개발" ~ "최초개발",
    사업구분 == "시행계획_최초개발" ~ "기승인",
    TRUE ~ 사업구분
