...
추진실적_사업목록 <- data_추진실적 %>%
  select(
    시군, 단위유역, 사업명, 협의일자, 준공예정연도,
    BOD_소진점, BOD_소진비점, TP_소진점, TP_소진비점,
    삭감방법, BOD_삭감량, TP_삭감량,
    할당일자, 착공연도, 준공여부, 협의상태, 협의연도, 사업구분
  ) %>%
  mutate(
    비고 = case_when(
      협의연도 > 기준년도 | 사업구분 == "기본계획_최초개발" ~ "제외",
      협의연도 < 기준년도 ~ "협의 부하량",
      협의연도 == 기준년도 ~ "사용 부하량"
    ),
    across(c(준공예정연도, 착공연도, 협의연도), as.character),
    # 사업취소인 경우 삭감량 음수로 지정
    BOD_삭감량 = ifelse(사업구분 == "사업취소", -1 * BOD_삭감량, BOD_삭감량),
    TP_삭감량 = ifelse(사업구분 == "사업취소", -1 * TP_삭감량, TP_삭감량),
    # 재협의인 경우 삭감량 0으로 지정
    BOD_삭감량 = ifelse(사업구분 == "재협의", 0, BOD_삭감량),
    TP_삭감량 = ifelse(사업구분 == "재협의", 0, TP_삭감량)
  ) %>%
  # 행순서 조정
...
추진실적_준공사업내역 <- data_추진실적 %>%
  select(
    시군, 단위유역, 사업명, 협의일자, 준공여부,
    BOD_소진점, BOD_소진비점, TP_소진점, TP_소진비점,
    삭감방법, BOD_삭감량, TP_삭감량, 사업구분
  ) %>%
