...
data_내부검토 <- data %>%
  ...
  mutate(across(all_of(num_cols), to_num0)) %>%
  ...
  # mutate(across(c(BOD.삭감량_합계:TP.소진_비점), ~ replace(., is.na(.), 0))) %>%
  # mutate(across(c(BOD.삭감량_합계:TP.소진_비점), as.numeric)) %>%
